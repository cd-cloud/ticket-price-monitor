...
class BrowserAutomation:
    ...
    def _looks_like_rate_limit(exc: Exception) -> bool:
        """Detect obvious rate-limit / anti-bot signals to stop retrying immediately."""
        text = str(exc).lower()
        markers = [
            "access denied",
            "forbidden",
    ...
    def _should_stop_after_no_fare(
        ...
        if attempt_index >= backend_count - 1:
            return True
        # Ctrip empty-result pages can be caused by session/direct-URL quirks.
        # Let the fallback backend confirm before treating it as a real no-fare.
